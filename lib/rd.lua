...
function rd.load(filename)
  ...
  function level:getrow(index)
    ...
    local row = {}
    row.level = self
    row.data = tget(self.data.rows, function(v) return v.row == index end)
    ...
    --save to level
    function row:save()
      tset(self.level.data.rows,function(v) return v.row == index end, self)
    end
  ...
  function level:getroom(index)
    ...
    function room:setpreset(beat,preset,state)
      ...
      self.level:addevent(beat,'SetVFXPreset',{rooms = self.level:roomtable(index), preset = preset, enable = state})
    end
  ...
  function level:setuprooms(beat)
    beat = beat or 0
    self:addevent(beat,'SetBackgroundColor',{rooms = {0,1,2,3}, backgroundType = 'Color', contentMode = 'ScaleToFill', color = 'FFFFFF00', image = {}, filter = 'NearestNeighbor', scrollX = 0, scrollY = 0})
  end
  ...
  function level:save(filename)
    ...
    for i,v in ipairs(self.fakeevents) do
      ...
      elseif v.type == 'updaterowx' then
        ...
        self:addevent(v.beat,'MoveRow',{
          row = v.row,
          target = 'WholeRow',
          customPosition = true,
          
          rowPosition = {
            getvalue(level.rows[v.row],'x',v.beat) + getvalue(level.rows[v.row],'room',v.beat)*852.2727,
            null
          },
      ...
      elseif v.type == 'updateroommode' then 
        ...
        self:addevent(v.beat,'SetRoomContentMode',{
